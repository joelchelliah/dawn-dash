...
function parseCliArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (arg === '--from-dump') {
      args.fromDump = true
    } else {
      console.error(`❌ Unknown argument: ${arg}`)
      console.error(
        'Usage: node parse-event-trees.js [--debug <event>] [--only <event>] [--dry-run] [--baseline <file>] [--from-dump]'
      )
      process.exit(1)
    }
  }
...
// Default input is events.json, generated by the external event-extraction tool and pasted in.
// --from-dump switches to events-from-dump.json, the output of the legacy in-repo pipeline
// (fetch-events-data-from-blightbane.js -> extract-events.js).
const EVENTS_FILE = path.join(
  __dirname,
  CLI_ARGS.fromDump ? '../data/events-from-dump.json' : '../data/events.json'
)
...
async function processEvents() {
  ...
  console.log(`📖 Reading events file (${path.basename(EVENTS_FILE)})...`)
