...
- **The React DevTools Chrome extension breaks Fast Refresh on this project.** With it enabled, an edit 404s on `/_next/static/webpack/<hash>.hot-update.json` and the terminal logs `⚠ Fast Refresh had to perform a full reload` — a _different_ hash each time, because the extension desyncs the Fast Refresh client from the server's compilation. The server is fine: the hot-update files are written to `.next/static/webpack/` correctly, the browser just asks for hashes that were never produced. It is **not** a stale `.next`, a stale tab, or a service worker, so `rm -rf .next`, restarting the server and unregistering workers all change nothing. Disable the extension at `chrome://extensions` and hot reload returns immediately. **It is still the right tool for profiling and INP work** (see `src/codex/specs-inp.md`) — so turn it on for a profiling session and back off afterwards, rather than leaving it enabled. To confirm this is the cause in seconds, open the site in incognito or another browser: working there means an extension, not the code.
...
- `npm run compress-images` - Regenerate every `public/og-image-<tool>.png` and `public/logo-<tool>.png` from the uncompressed originals in `image-sources/`. **The originals are the only editable copies** — `public/` holds disposable output, so never edit an image there (see Image sources)
