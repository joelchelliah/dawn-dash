...
## Spec 19 — Replace the hand-rolled horizontal spacing passes with `d3-flextree` — ✅ COMPLETED (final visual check pending)

> **Notes:** Landed as spec'd. `adjustHorizontalNodeSpacing` (name kept; EventTree's `tree()` pre-layout is gone — flextree owns initial x *and* y placement) now runs `flextree({ nodeSize: (d) => [width(d) + minGap, VERTICAL_SPACING_DEFAULT] })` followed by one `centerMultiParentChildren` pass. Conventions verified from the flextree source before wiring: `x` is the slot **center** (`left = x − xSize/2`, root at 0 — same as `tree()`), `y` is the slot top, but with constant `ySize` it equals `depth * VERTICAL_SPACING_DEFAULT`, numerically identical to the old pre-layout, so `adjustVerticalNodeSpacing` needed no changes; `layout(tree)` accepts a plain `d3.hierarchy` and writes x/y back onto the original nodes. Deleted: the three repair passes, `logging.ts`, the sweep caps + `MIN_SHIFT_THRESHOLD`, `buildParentGroups`, `getAllChildrenForParentGroup`, grouping's `findNodeById`, and `calculateGapBetweenNodes` — net **−445 lines** (folder: 1,118 → 632). Added `d3-flextree` + `@types/d3-flextree` (DefinitelyTyped types exist). **Automated verification** (temp test, deleted after): the full pipeline run over all 183 events × 3 levels of detail × both looping/continues toggles asserted zero same-depth overlaps (min-gap respected), finite coordinates, and depth-aligned strictly-increasing rows — all passed. Caveat for the temp-test approach: `next/jest` hard-blocks transforming `node_modules` (the d3 ESM packages), so the existing `transformIgnorePatterns` in `jest.config.ts` is ineffective; the test ran via a temporary wrapper config that replaced the pattern post-generation. `npm run verify` + `npm run build` (all event pages prerendered) pass. Remaining: the spec's broad visual QA (refChildren-heavy/looping events, compact + detailed, zoom levels).
>
> **Follow-up (visual QA round 1):** shared-children fans (children of A that are also refChildren of B) were no longer centered under the midpoint of their parents — `centerMultiParentChildren` moved each child individually, so siblings at the minimum gap blocked each other. Fixed twofold: (1) the pass now moves sibling groups sharing the same parent set **as one unit**, with the moving group excluded from the obstacle clamp; (2) a new bottom-up `centerParentsOverSharedChildren` pass moves parent groups (nodes only, subtrees stay) over their children when the child-side shift is clamped. Ground truth came from running the old engine (still at HEAD) side-by-side over every shared-fan constellation: after the fix, 16/24 center as well as or better than old (many at 0px where old was 15–300px off). **Known accepted limitation** (user decision: ship as-is): 8 *crossing/interleaved-fan* constellations stay 170–600px off-center where old converged to ~0 — two mutually-coupled fans can't both be satisfied by deterministic clamped passes; the old engine solved these via its iterative push-apart-and-recenter sweeps. Affected events listed in the folder README. The overlap-invariant suite (all events × all settings) still passes with the new passes.
...
6. ~~Spec 3 (tree unification, step by step)~~ (done) → ~~Spec 4 (spacing decomposition)~~ (done) → ~~Spec 19 (flextree swap)~~ (done).
