...
**Pause after task 7 for the pre-launch verification pass (tasks 10–14).** That's the point where the
tool is publicly visible and everything is checkable at once in the browser. **Task 14 is the
exception to "in the browser"** — it's manual Search Console work against the *deployed* site, so it
waits until the launch commit is actually in production.

**Tasks 8–9 are doc-only and have no visible effect** — don't mistake them for broken steps.

**Task 15 is post-launch and explicitly not a blocker** — a pre-existing service-worker issue found
...
### 7. Remove `unlisted: true` — the launch switch — COMPLETED
...
**Outcome:** the one line is gone; nothing else in the entry changed. `npm run verify`,
`npm run build` and `npm run check-sw` all pass (**105 precached URLs, all resolving** — unchanged,
since the images landed in task 3).

Confirmed in the built HTML rather than assumed:

- `.next/server/pages/booty.html` now carries `<link rel="canonical" href="https://www.dawn-dash.com/booty">`
  and **zero** occurrences of `noindex`.
- The JSON-LD block is emitted and its `image` points at `logo-booty.png` — the file task 3 added.
- `.next/server/pages/index.html` references `landing-booty.webp`, so the landing tile renders from
  a real asset.

**Still needs the user's eyes** (task 10): the desktop hover panel vs. the mobile card, and side-menu
ordering — those never co-render, so the build output can't stand in for looking at them.

## Documentation tasks

**No visible effect** — these are doc-only and must not be mistaken for broken steps.

### 8. Update `src/codex/CLAUDE.md` for four tools — COMPLETED
...
**Outcome:** title now reads `# src/codex/ — Cardex, Skilldex, Eventmaps, and Booty`; the opening
line says four tools and names `booty.tsx`. The two treasure JSONs were added to Key files as a
single combined entry directly under `event-trees.json`, phrased to match it (regenerated by
`scripts/sync-treasures.js`, never hand-edit, external tool as the source). Invariants untouched.

### 9. Document Booty in the root `CLAUDE.md` — COMPLETED
...
**Outcome:** all three edits made — "five tools" → six (and Booty added to both that sentence and
the `/src` feature-directory line, which also said `codex/ (Cardex + Skilldex + Eventmaps)`), a
fifth numbered Main Features entry inserted before Scoring (renumbering Scoring to 6), and
`sync-treasures.js` documented in Data Synchronization.

Two details verified against the source rather than taken from the spec:

- **`sync-all` runs three scripts, not two** — `sync-treasures`, `sync-events`, *and*
  `sync-talents-preflight` (`scripts/sync-all.js:17-21`). The root file didn't mention `sync-all` at
  all before, so it's documented with its actual contents.
- **The fail-fast behaviour is real** — `sync-treasures.js:33-38` throws when
  `scripts/data/treasures.json` is missing, with a message naming the external extraction tool.

The `/src` feature-directory line was **not** in the spec's list of edits; it carried the same stale
tool count and would have contradicted the line above it, so it was updated too.
...
### 14. Register the page with Google Search Console — after deploying

**Everything below happens against the live site**, so it can only be done once the launch commit is
deployed to production. Nothing here is a code change — it's all manual, in the Search Console UI at
[search.google.com/search-console](https://search.google.com/search-console) for the
`dawn-dash.com` property.

**The sitemap does not need regenerating.** `/booty` went in during task 5 and is already committed
in `public/sitemap.xml` (218 URLs, entry at line 34, priority `0.8`). The only thing left is telling
Google the sitemap changed. Re-running `node scripts/generate-sitemap.js` would only restamp every
`lastmod` to today — harmless, but it produces a 218-line diff for no benefit, so skip it.

The steps, in order:

1. **Confirm the page is actually indexable in production.** Fetch `https://www.dawn-dash.com/booty`
   and check the served HTML has `<link rel="canonical" href="https://www.dawn-dash.com/booty">` and
   **no** `noindex`. This was verified in the build output for task 7, but a stale CDN copy or a
   deploy that didn't pick up the registry change would show up here — and submitting a `noindex`
   page to Search Console gets it recorded as excluded, which is slower to undo than to avoid.
2. **Resubmit the sitemap.** Search Console → **Sitemaps** → the existing `sitemap.xml` entry →
   resubmit (or re-enter `sitemap.xml` and Submit). This is what prompts a re-read; Google otherwise
   recrawls it on its own schedule, which can be days. Expect "Discovered URLs" to tick from 217 to
   218 — not immediately, it updates after Google actually processes the file.
3. **Request indexing for `/booty` directly.** Search Console → **URL Inspection** → paste
   `https://www.dawn-dash.com/booty` → **Request Indexing**. Sitemap submission alone just queues
   the URL for eventual discovery; this is the part that gets a single new page crawled in hours to
   days rather than weeks. The inspection result should say the URL is on Google *or* "URL is not on
   Google" with **no crawl blockers** — if it reports `noindex`, stop and go back to step 1, since
   that means production is serving something different from the build.
4. **Check `robots.txt` isn't in the way.** `public/robots.txt` is `Disallow:` (empty — allows
   everything) and points at `https://www.dawn-dash.com/sitemap.xml`, so no change is needed. Worth
   one look only because a blocked path makes step 3 fail confusingly.
5. **Come back in a few days** and confirm in **Pages** (or a re-inspection) that `/booty` is
   indexed. If it's still "Discovered – currently not indexed" after a week or so, that's normal for
   a new page on a small site and not something to fix by re-submitting repeatedly.

**Note the `www` host.** The canonical, the sitemap `<loc>` entries and `robots.txt` all use
`https://www.dawn-dash.com`. Inspect the `www` URL in Search Console — inspecting the bare
`dawn-dash.com` form reports on a different URL than the one being indexed and will read as
confusing rather than as an error.

There is nothing to record in the repo for this task; it's operational, and the sitemap side of it
is already done in code.
...
### 15. Stop precaching og-images and logos (`publicExcludes`)
